...
def cm_to_metrics(cm: np.ndarray):
    ...
    f1_c = np.divide(
        ...
        out=np.zeros_like(tp), where=(prec_c + recall_c) > 0
    )
...
class ConfusionMatrixMetrics(tf.keras.metrics.Metric):
    ...
    def result(self):
        ...
        row_sum = tf.reduce_sum(cm, axis=1)          # true counts per class
        col_sum = tf.reduce_sum(cm, axis=0)          # pred counts per class
        ...
        # balanced accuracy = macro recall over present classes
        mask_true = row_sum > 0
        bal_acc = tf.cond(
        ...
        # macro precision over predicted classes
        mask_pred = col_sum > 0
        macro_prec = tf.cond(
        ...
        # macro recall is same as balanced accuracy here
        macro_rec = bal_acc

        # macro F1 over classes that exist and have (prec+rec)>0
        f1_c = tf.where(
            (prec_c + recall_c) > 0,
        ...
        # Return a dict so Keras can log multiple values from one Metric
        return {
            "accuracy": acc,
...
class ConfMatSaver(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        #logs["bal_acc_epoch"] = m_train["balanced_accuracy"]
        #logs["val_bal_acc_epoch"] = m_val["balanced_accuracy"]
...
class DynamicAnchorWarmup(tf.keras.callbacks.Callback):
    ...
    def _get_concept_score(self, logs: dict):
        """
        Returns a scalar where LOWER means better concept quality.
        """
        loss_name = getattr(self.model, "concept_loss_name", None)  # "bce" or "mse"
        if loss_name == "bce":
            # Prefer macro F1 if present (higher is better -> convert to error)
            f1 = logs.get("val_concept_f1_macro", None)
            if f1 is not None:
                f1 = float(f1)
                #return 1.0 - f1, "1 - val_concept_f1_macro"

            # Fallback: macro BCE (lower is better)
        ...
        # Continuous concepts: macro MSE (lower is better)
...
# ---------------------------------------------------------
# P(A_m | k) tracking callback (optional)
# ---------------------------------------------------------
class PAmkEMAvsRawCallback(tf.keras.callbacks.Callback):
    ...
    Requires that the model keeps:
      - model._last_batch_attr_preds (B,M)
      - model._last_train_batch_labels (B,)
      - model.concept_class_priors.P_pred_ema
    """
    ...
    def _compute_PAmk_raw(self, model):
        last_attr = model._last_batch_attr_preds     # [B,M]
        last_labels = model._last_train_batch_labels # [B]
        ...
        concatenated_attributes = tf.expand_dims(last_attr, axis=-1)    # [B,M,1]
        attribute_class_probs = concatenated_attributes * labels_oh     # [B,M,K]
        P_A_mk_batch = tf.reduce_sum(attribute_class_probs, axis=0)     # [M,K]

        class_count = tf.math.bincount(
            labels, minlength=self.K, maxlength=self.K, dtype=tf.float32
        )
        mask = class_count == 0
    ...
    def _plot_heatmap(self, matrix, epoch, title, suffix):
        ...
        plt.savefig(
            os.path.join(self.save_dir,
                         f"p_amk_{suffix}_epoch_{epoch + 1}.png"),
            dpi=150
        )
        plt.close()
...
class RunSummarySaver(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if "val_bal_acc_epoch" in logs:
            self.best["val_bal_acc"] = max(
                self.best["val_bal_acc"], logs["val_bal_acc_epoch"]
            )

        # concept quality (AwA2)
        if "val_concept_mse_macro" in logs:
            self.best["val_concept_mse_macro"] = min(
                self.best["val_concept_mse_macro"], logs["val_concept_mse_macro"]
            )

        # gamma interpretability proxy
        if "val_loss_prior_anch" in logs:
            self.best["val_loss_prior_anch"] = min(
                self.best["val_loss_prior_anch"], logs["val_loss_prior_anch"]
            )

    def on_train_end(self, logs=None):
        import json, os
        with open(os.path.join(self.save_dir, "run_summary.json"), "w") as f:
