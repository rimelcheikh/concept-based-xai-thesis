...
class CUBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.path_transform == None:
            ...
            try:
                idx = img_path.split('/').index('images')#('CUB_200_2011')
...
def load_data(
    ...
    return loader, SELECTED_CONCEPTS, CLASS_NAMES
...
def generate_data(config, data_dir, rerun=False,):
    
    root_dir = data_dir + "/CUB_200_2011/" #config.get("root_dir")
    base_dir = os.path.join(root_dir, 'class_attr_data_10')
    
    seed = config.get("seed")
    seed_everything(seed)
    
    resol = config.get("image_size")
    ...
    if not config.get("output_dataset_vars"):
        return train_dl, val_dl, test_dl, imbalance
    
    
    
    return train_dl, val_dl, test_dl, imbalance, SELECTED_CONCEPTS, CLASS_NAMES, concept_group_map, {}
