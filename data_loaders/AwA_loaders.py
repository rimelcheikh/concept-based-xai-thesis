...
def load_CIFAR_AwA_data(unbalanced, config):
...
def load_AwA_data_all(cv, config):
    ...
    def process_image(file_info):
        c, file = file_info
        img_path = os.path.join(dataset_dir, c, file)
        try:
            img = load_img(img_path, target_size=(224, 224))
            img_array = img_to_array(img)
            return (img_array, label_to_idx_awa[c], mat_pd_all[c].values / 100)
        except:
    ...
    for c in classes:
        ...
        if config.get("n_samples_per_class") is not None:
            files = os.listdir(class_dir)[:config.get("n_samples_per_class")]  
        else:
            files = os.listdir(class_dir)
        
        file_list.extend([(c, f) for f in files])
    ...
    for result in results:
        if result:
            img_array, label, annotation = result
    ...
    if n_per_class is not None:
        ...
        keep_idx = []
        for clas in np.unique(labels):
            idx_cls = np.where(labels == clas)[0]
            if len(idx_cls) > n_per_class:
                sel = rng.choice(idx_cls, size=n_per_class, replace=False)
            else:
                sel = idx_cls
            keep_idx.extend(sel)
    ...
    if cv == False:
        X_train, X_test, y_train, y_test, a_train, a_test = train_test_split(
                                    images, labels, annotations, test_size=test_split, stratify=labels, random_state=seed)
        ...
        return  images, None, labels, None, annotations, None, mat_pd_awa, classes, list(mat_GT.index), label_to_idx_awa, mat_GT
        
    
    
def load_AwA_data_17(cv, config):
    ...
    classes = []
    for c in sorted(os.listdir(data_dir+'/AwA/JPEGImages'))[1:]:
    ...
    for c in classes:
        counter = 0
        class_dir = os.path.join(data_dir+'/AwA/JPEGImages', c)
